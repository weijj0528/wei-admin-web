<template>
  <ListLayout title="租户列表" :page="pagination" @page-change="handlePageChange" @size-change="handleSizeChange">
    ...
    <template #actions>
      <el-button v-permission="['system:tenant:save']" type="primary" :icon="Plus" @click="handleAdd">新建租户</el-button>
    </template>
    <el-table :data="tableData" v-loading="loading" stripe height="100%">
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          <el-button v-if="row.status !== 0" v-permission="['system:tenant:enable']" link type="success" @click="toggle(row, true)">启用</el-button>
          <el-button v-else v-permission="['system:tenant:disable']" link type="warning" @click="toggle(row, false)">禁用</el-button>
          <el-button v-permission="['system:tenant:update']" link type="primary" @click="handleEdit(row)">编辑</el-button>
          <el-button v-permission="['system:tenant:delete']" link type="danger" @click="handleDelete(row)">删除</el-button>
        </template>
      </el-table-column>
